...
import qing.yun.hui.poitool.util.excel.enums.IExcelCommonEnum;
import qing.yun.hui.poitool.util.excel.exception.ExcelCellTypeException;
import qing.yun.hui.poitool.util.excel.exception.ExcelEnumTypeException;
import qing.yun.hui.poitool.util.excel.exception.ExcelTitleParseException;

/***
 ** @Description: excel 导入
 ** @author: qing.yunhui
 ** @email: 280672161@.qq.com
 ** @dateTime: Nov 20, 2015 4:26:35 PM
 ** @version: V1.0
 ***/
public class SimpleExcelImport implements IExcelImport{
	...
	public <T> List<T> execute(InputStream is, Class<T> clazz) throws Exception  {
		...
		//工作薄的信息@1
		//ExcelSheet sheetInfo = parseAnnotationExcelSheet(clazz);
		//根据注解的名字来获取需要解析哪些数据@2
		//parseAnnotationExcelColumn(clazz);
		
		ExcelSheet sheetInfo = eap.getSheetInfo();
	...
	private void parseCell(Cell cell,Object instance,Map.Entry<String,PropertyDescriptor>  entry) throws Exception{
		//获取字段的注解信息
		ExcelColumn annColumn= eap.getFieldAnnInfo().get(entry.getKey());
		if(annColumn == null){
	 ...
	 * @param ExcelColumn 字段的注解
	 * @return
	 * @throws ExcelCellTypeException 
	 * @throws ExcelEnumTypeException 
	 */
	private Object transformType(Object source,Class<?> target,ExcelColumn ExcelColumn,Cell cell) throws ExcelCellTypeException, ExcelEnumTypeException{
		...
		if(source.getClass() == String.class){
			if(ExcelColumn.enumType()!=ExcelNullEnum.class){//枚举对象的处理
				return enumProcess(source,ExcelColumn.enumType());
